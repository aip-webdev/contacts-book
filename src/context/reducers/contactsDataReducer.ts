...
type ContactsActions = AddNewGroupAction | AddNewContactDataAction | AddNewContactAction | RemoveContactAction | FetchContactsSuccessAction

export const contactsDataReducer: Reducer<IContacts [], ContactsActions> = (state, action) => {
    switch (action.type) {
        case 'ADD_NEW_GROUP': {
            const contactData: IContacts = state
                .filter((contactsData: IContacts) => contactsData.userId === action.payload.userId)[0]
